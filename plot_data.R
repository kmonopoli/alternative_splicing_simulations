...
#              c(5,5,5,5,5))
# 
# grid.arrange(sp, hplt, jeeplt, tbl, layout_matrix = lay)
...
spltype = c(ft$splice_type_read,ft$splice_type_read,ft$splice_type_read)
...
p <- ggplot(segment_data, aes(x = x, y = y, xend = xend, yend = yend))+
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend, color = splice_type))#,color = segment_data$splice_type)
