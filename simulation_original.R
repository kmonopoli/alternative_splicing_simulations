...
# Attributes (not running from command line)
# DIR = "/Users/kmonopoli/Google_Drive/research/eLifeScripts_forKMonopoli/simulations/runoncluster/"
# D_dists = 3
# expression_levels = 4
...
# introns = c(15)#c(seq(0.04,0.09,by=0.02), seq(0.1,1,by=0.1), seq(1,50,by=2))
	...
	print(paste0("minute ",labeling," - distance ",D_dist," - expression ",expression_level," - half-life ",half_life,"..."))
	  ...
	  # if(intron %% 1 == 0){ flush.console() }
          ...
          ################ ################ ################ SIMULATE START ################## ################ ################
            ...
            results = list(reads_formatted, spliced_num, unspliced_num, intron_num, start_pos$spliced)
          ################ ################ ################ SIMULATE END ################## ################ ################
          
          
          
          reads = results[[1]]
          # reads: dataframe in read format - can use for MISO - chr is intron length
